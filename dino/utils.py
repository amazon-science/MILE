...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        for end_idx in idx_crops:
            ...
            # print(x[start_idx].shape, "times ", end_idx - start_idx +1,  "-> ", _out.shape, "acc", output.shape)
            start_idx = end_idx
...
class BLRP(nn.Module):
    def __init__(self, backbone, latent_cross, dual_latent_cross, num_global_crops, num_local_crops, final_projection=None, gate_tanh = False, dual_gate_tanh = False, bi_directional = False, union_latent_keys = False, backward_dual_latent_cross = False, cross_wi_patch_e = False, ignore_cls_in_lxs = False, cross_wi_registers = False,  arch = None, dinov2_force_cls_in_lxs = False):
        ...
        self.dual_latent_cross = dual_latent_cross # condition view output with latent
    ...
    def forward(self, x, output_type=None):   
        ...
        for view in x:
            if self.cross_wi_patch_e:
                ...
                # output = [self.backbone.backbone.forward_features(crop_view.unsqueeze(0)) for crop_view in view]
                # output = [torch.cat([crop["x_norm_clstoken"].unsqueeze(1), crop["x_norm_regtokens"]], dim=1) for crop in output]
